...
namespace Fall2024_Assignment3_jbmcclenny.Controllers
{
    public class MovieActorController : Controller
    {
        ...
        {
            var applicationDbContext = _context.MovieActor.Include(ma => ma.Movie).Include(ma => ma.Actor);
        ...
        public async Task<IActionResult> Create([Bind("Id,MovieId,ActorId")] MovieActorViewModel movieActor)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,MovieId,ActorId")] MovieActorViewModel movieActor)
        {
            if (id != movieActor.Id)
            {
                return NotFound();
